...
class VAEGPLVMModel(bm.BaseModel):
    def __init__(self, config):
        ...
        self.opt_trainer_encoder = None
        self.opt_trainer_decoder = None
    ...
    def build_model(self):
        ...
        with tf.name_scope("kernels"):
            log_amplitude_gp = tf.Variable(tf.zeros([t_vae_q], dtype=self.dtype), name="log_amplitude_latent")
        ...
        self.t_x_sample = self.t_encoder.sample()
        self.t_z = tf.transpose(self.gp.predict(self.t_x_sample, diag_covariance=True).sample())
        ...
        with tf.name_scope("trainers"):
            ...
            self.opt_trainer_encoder = tf.contrib.opt.NadamOptimizer(
                learning_rate=self.config["learning_rate_mlp"]).minimize(
                -self.t_avg_elbo_loss,
                var_list=tf.trainable_variables("encoder"))
            self.opt_trainer_decoder = tf.contrib.opt.NadamOptimizer(
                learning_rate=self.config["learning_rate_mlp"]).minimize(
                -self.t_avg_elbo_loss,
...
class VAEGPLVMRegressionModel(bm.BaseModel):
    def __init__(self, config):
        super(VAEGPLVMRegressionModel, self).__init__(config)
    ...
    def build_model(self):
        ...
        self.t_z = tf.transpose(self.gp.predict(self.t_x, diag_covariance=True).sample())
        ...
        with tf.name_scope("elbo"):
            ...
            self.t_avg_kl_global = weight * self.gp.regularization
            self.t_avg_elbo_loss = \
                self.t_avg_reco - self.t_avg_kl_global - self.hyperprior - \
                tf.reduce_sum(tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)) / \
                tf.constant(self.config["num_data_points"], dtype=self.dtype)
